...
void solve() {
  queue<tuple<Pos, Pos, int, string>> q; // 빨강공 좌표, 파랑공 좌표, 단계
  int dx[4] = {1, -1, 0, 0};
  int dy[4] = {0, 0, 1, -1};
  char dc[4] = {'D', 'U', 'R', 'L'};

  string command = "";
  q.push(make_tuple(r, b, 0, command));
  ...
  while(!q.empty()){
    ...
    for(int i = 0; i < 4; i++){
      ...
      q.push(make_tuple(nextR, nextB, step + 1, nc));
    }
  }
  cout << -1;
}
